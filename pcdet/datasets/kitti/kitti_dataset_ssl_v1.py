...
###weak aug 只进行一个变换, strong aug随即进行进行3个变换

class KittiDatasetSSL_v1(DatasetTemplate):
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            data_dict_unlabeled = self.get_item_single_unlabeled(info_unlabeled, no_db_sample=True)
    ...
    def get_item_single_unlabeled(self, info, no_db_sample=False):
        sample_idx = info['point_cloud']['lidar_idx']
        ...
        data_dict = self.prepare_data_unlabeled(data_dict=input_dict, no_db_sample=no_db_sample)
        # if isinstance(data_dict, list):
        #     print(data_dict)
        data_dict['image_shape'] = img_shape
        return data_dict
    
    def prepare_data_unlabeled(self, data_dict, no_db_sample=False):
        """
        Args:
            data_dict:
                points: (N, 3 + C_in)
        ...
        if self.training:
            ...
            gt_boxes_ema, points_ema, _ = random_flip_along_x(gt_boxes_ema, points_ema, enable_=data_dict['flip_x'])
            gt_boxes_ema, points_ema, _ = random_flip_along_y(gt_boxes_ema, points_ema, enable_=data_dict['flip_y'])
            #做了逆变换, ema 再单独做一次变换  useless
            '''
            random_choice = np.random.randint(0,3)
            if random_choice == 0:
                gt_boxes_ema, points_ema, noise_scale = global_scaling(gt_boxes_ema, points_ema, [0.95, 1.05])
                data_dict['scale'] = data_dict['scale']*(1/noise_scale)
            elif random_choice == 1 :
                gt_boxes_ema, points_ema, noise_rotation = global_rotation(gt_boxes_ema, points_ema, [-0.78539816, 0.78539816])
                data_dict['rot_angle'] = data_dict['rot_angle']+(-noise_rotation)
            elif random_choice == 2 :
                gt_boxes_ema, points_ema, enable = random_flip_along_x(gt_boxes_ema, points_ema)
                data_dict['flip_x'] = data_dict['flip_x']^enable
            '''
            #print(data_dict['points'].shape)
            sample_point = True
            if sample_point:
                from ...utils.common_utils import sample_points
                data_dict = sample_points(data_dict)
                #print(data_dict['points'].shape)
