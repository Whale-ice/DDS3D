...
class VoxelRCNNHead(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, num_class=1,point_cloud_range=[0, -40, -3, 70.4, 40, 1], voxel_size=[0.05,0.05,0.1], **kwargs):
    ...
    def forward(self, batch_dict, disable_gt_roi_when_pseudo_labeling=False):
        ...
        targets_dict = self.proposal_layer(
            batch_dict, nms_config=self.model_cfg.NMS_CONFIG['TRAIN' if self.training  and not disable_gt_roi_when_pseudo_labeling==False else 'TEST']
        )
        ...
        if self.training or self.print_loss_when_eval:
            ...
            targets_dict['rois'] = batch_dict['rois']
