...
class PointRCNN_ssl(Detector3DTemplate):
    def __init__(self, model_cfg, num_class, dataset):
        super().__init__(model_cfg=model_cfg, num_class=num_class, dataset=dataset)
        # something changes so need deep copy
        model_cfg_copy = copy.deepcopy(model_cfg)
        dataset_copy = copy.deepcopy(dataset)
        self.point_rcnn = PointRCNN(model_cfg=model_cfg, num_class=num_class, dataset=dataset)

#  teacher
        self.point_rcnn_ema = PointRCNN(model_cfg=model_cfg_copy, num_class=num_class, dataset=dataset_copy)
        for param in self.point_rcnn_ema.parameters():
            param.detach_()
        self.add_module('point_rcnn', self.point_rcnn)
        self.add_module('point_rcnn_ema', self.point_rcnn_ema)
    ...
    def forward(self, batch_dict):
        if self.training:
            ...
            with torch.no_grad():
                # self.point_rcnn_ema.eval()  # Important! must be in train mode
                for cur_module in self.point_rcnn_ema.module_list:
                    try:
                        batch_dict_ema = cur_module(batch_dict_ema, disable_gt_roi_when_pseudo_labeling=True)
                    except:
                        batch_dict_ema = cur_module(batch_dict_ema)
                pred_dicts, recall_dicts = self.point_rcnn_ema.post_processing(batch_dict_ema,
            ...
            for cur_module in self.point_rcnn.module_list:
                batch_dict = cur_module(batch_dict)

            disp_dict = {}
            loss_point, tb_dict = self.point_rcnn.point_head.get_loss(tb_dict, scalar=False)
            loss_rcnn_cls, loss_rcnn_box, tb_dict = self.point_rcnn.roi_head.get_loss(tb_dict, scalar=False)
            ...
            loss_rcnn_cls = loss_rcnn_cls[labeled_mask, ...].sum() #+loss_rcnn_cls[unlabeled_mask, ...]
            ...
            pred_dicts, recall_dicts = self.point_rcnn.post_processing(batch_dict)
    ...
    def update_global_step(self):
        ...
        for ema_param, param in zip(self.point_rcnn_ema.parameters(), self.point_rcnn.parameters()):
